# ---------------------------------------------------------------------------
# ---- 2.2.10 Нарастающий итог по месяцам с начала года. Среднесуточный. ----
# ---------------------------------------------------------------------------
...
query_2_2_10_1 = ("""select 'tgt', count(0)
...
query_2_2_10_2 = ("""select 	a.hcode_id, a.hcode_name, a.hcode_unit_name, a.org_id, a.dor_kod, a.date_type_id, a.metric_type_id, a.cargo_type_id, 
		a.val_type_id, a.unit_id, a.dt, a.value, a.ss, a.duch_id, a.nod_id, a.dir_id, a.vids_id, a.kato_id, a.depo_id, a.dep_id
	from (
		select 	ind.hcode_id, ind.hcode_name, ind.hcode_unit_name, ind.org_id, ind.dor_kod, 4 as date_type_id, ind.metric_type_id, ind.cargo_type_id, 
				5 as val_type_id, ind.unit_id, dt, ind.value as value, ind.ss, ind.duch_id, ind.nod_id, ind.dir_id, ind.vids_id, ind.kato_id, ind.depo_id, ind.dep_id
			from ( 	
					select * from dm_stg.calc_src_indicators_t 
...
left join ( -- Секция выявления более приоритетных дублей
	select * from dm_stg.calc_src_indicators_t ind 
		join (select hcode_id as hid, src, priority from dm_lgc.m_hcode_priority_t) hcp on ind.hcode_id = hcp.hid
		join (select hcode_id as hid, src, priority from dm_lgc.m_hcode_priority_t where src = 'CALC') hcpr on hcp.hid = hcpr.hid
		join (select distinct id, group_type from dm_stg.d_hcode_t) hcd
			on ind.hcode_id = hcd.id
...
# -- Сверка кол-ва записей 
query_2_2_10_3 = ("""select 'tgt', count(0)
	from dm_rep.dm_all_indicators_v clc
	join (select distinct id, group_type from dm_stg.d_hcode_t) hcd
		on clc.hcode_id = hcd.id
where 	calc_rule = 'growplan'
...
# -- Сравнение всех записей в таблицах
query_2_2_10_4 = ("""select 	a.hcode_id, a.hcode_name, a.hcode_unit_name, a.org_id, a.dor_kod, a.date_type_id, a.metric_type_id, a.cargo_type_id, 
		a.val_type_id, a.unit_id, a.dt, a.value, a.ss, a.duch_id, a.nod_id, a.dir_id, a.vids_id, a.kato_id, a.depo_id, a.dep_id
	from (
		select 	ind.hcode_id, ind.hcode_name, ind.hcode_unit_name, ind.org_id, ind.dor_kod, 4 as date_type_id, ind.metric_type_id, ind.cargo_type_id, 
				5 as val_type_id, ind.unit_id, ind.dt, 
				case
					when
						ind.hcode_id between '00045' and '00054' 
					then 
						((ind.value + ind2.value)/
						(case when ind.value is not null then 1 else 0 end + case when ind2.value is not null then 1 else 0 end))::numeric(20,6) 
					else
						...
						+ (case when ind2.value is not null then 1 else 0 end)*dte2.month_length))::numeric(20,12) 
					end as value, 
...
and ind.dep_id = ind2.dep_id
	...
	join dm_stg.d_date_t dte on dte.dt = ind.dt
...
select 	clc.hcode_id, clc.hcode_name, clc.hcode_unit_name, clc.org_id, clc.dor_kod, clc.date_type_id, clc.metric_type_id, clc.cargo_type_id, 
		clc.val_type_id, clc.unit_id, clc.dt, clc.value::numeric(20,12), clc.ss, clc.duch_id, clc.nod_id, clc.dir_id, clc.vids_id, clc.kato_id, clc.depo_id, clc.dep_id
	from dm_rep.dm_all_indicators_v clc
	join (select distinct id, group_type from dm_stg.d_hcode_t) hcd
		on hcd.id = clc.hcode_id
where   calc_rule = 'growplan'
...
# -- Сверка кол-ва записей 
query_2_2_10_5 = ("""select 'tgt', count(0) from dm_rep.dm_all_indicators_v clc
join (select distinct id, group_type from dm_stg.d_hcode_t) hcd
	on clc.hcode_id = hcd.id
where 	calc_rule = 'growplan'
	and val_type_id = 5
...
# -- Сравнение всех записей в таблицах
query_2_2_10_6 = ("""select 	a.hcode_id, a.hcode_name, a.hcode_unit_name, a.org_id, a.dor_kod, a.date_type_id, a.metric_type_id, a.cargo_type_id, 
		a.val_type_id, a.unit_id, a.dt, a.value, a.ss, a.duch_id, a.nod_id, a.dir_id, a.vids_id, a.kato_id, a.depo_id, a.dep_id
	from (
		select 	ind.hcode_id, ind.hcode_name, ind.hcode_unit_name, ind.org_id, ind.dor_kod, 4 as date_type_id, ind.metric_type_id, ind.cargo_type_id, 
				5 as val_type_id, ind.unit_id, ind.dt, 
				case
					...
					then 
						...
						case when ind3.value is not null then 1 else 0 end))::numeric(20,8) 
					else
						...
						+ (case when ind3.value is not null then 1 else 0 end)*dte3.month_length))::numeric(20,8) 
					end as value,  
				ind.ss, ind.duch_id, ind.nod_id, ind.dir_id, ind.vids_id, ind.kato_id, ind.depo_id, ind.dep_id
			from ( 	
					select * from dm_stg.calc_src_indicators_t 
...
# -- Сверка кол-ва записей 
query_2_2_10_7 = ("""select 'tgt', count(0)
		from dm_rep.dm_all_indicators_v clc
		join (select distinct id, group_type from dm_stg.d_hcode_t) hcd
			on clc.hcode_id = hcd.id
where 	calc_rule = 'growplan'
...
# -- Сравнение всех записей в таблицах
query_2_2_10_8 = ("""select 	a.hcode_id, a.hcode_name, a.hcode_unit_name, a.org_id, a.dor_kod, a.date_type_id, a.metric_type_id, a.cargo_type_id, 
		a.val_type_id, a.unit_id, a.dt, a.value, a.ss, a.duch_id, a.nod_id, a.dir_id, a.vids_id, a.kato_id, a.depo_id, a.dep_id
	from (
		select 	ind.hcode_id, ind.hcode_name, ind.hcode_unit_name, ind.org_id, ind.dor_kod, 4 as date_type_id, ind.metric_type_id, ind.cargo_type_id, 
				5 as val_type_id, ind.unit_id, ind.dt, 
				case
					when
						ind.hcode_id between '00045' and '00054' 
					then 
						((ind.value + ind2.value + ind3.value + ind4.value)/		
						(case when ind.value is not null then 1 else 0 end + case when ind2.value is not null then 1 else 0 end + 
						case when ind3.value is not null then 1 else 0 end + case when ind4.value is not null then 1 else 0 end))::numeric(20,8) 
					else
						((ind.value*dte.month_length + ind2.value*dte2.month_length + ind3.value*dte3.month_length 
						+ ind4.value*dte4.month_length)/
						((case when ind.value is not null then 1 else 0 end)*dte.month_length 
						+ (case when ind2.value is not null then 1 else 0 end)*dte2.month_length 
						+ (case when ind3.value is not null then 1 else 0 end)*dte3.month_length 
						+ (case when ind4.value is not null then 1 else 0 end)*dte4.month_length))::numeric(20,8) 
					end as value,   
				ind.ss, ind.duch_id, ind.nod_id, ind.dir_id, ind.vids_id, ind.kato_id, ind.depo_id, ind.dep_id
...
and ind.dep_id = ind4.dep_id
	...
	join dm_stg.d_date_t dte2 on dte2.dt = ind2.dt
	join dm_stg.d_date_t dte3 on dte3.dt = ind3.dt
	join dm_stg.d_date_t dte4 on dte4.dt = ind4.dt
	where ind.%s
	) a
left join ( -- Секция выявления более приоритетных дублей
	select * from dm_stg.calc_src_indicators_t ind 
...
# -- Сверка кол-ва записей 
query_2_2_10_9 = ("""select 'tgt', count(0)
		from dm_rep.dm_all_indicators_v clc
join (select distinct id, group_type from dm_stg.d_hcode_t) hcd
	on clc.hcode_id = hcd.id
where 	calc_rule = 'growplan'
...
select 'src', count(0)	from (
	...
	join (select distinct id, group_type from dm_stg.d_hcode_t) hcd
		on ind.hcode_id = hcd.id
	where 	ind.metric_type_id in (12,2,18)
		and ind.val_type_id = 7
		and ind.date_type_id = 4
		and hcd.group_type = 'среднесуточный'
...
# -- Сравнение всех записей в таблицах
query_2_2_10_10 = ("""select 	a.hcode_id, a.hcode_name, a.hcode_unit_name, a.org_id, a.dor_kod, a.date_type_id, a.metric_type_id, a.cargo_type_id, 
		a.val_type_id, a.unit_id, a.dt, a.value, a.ss, a.duch_id, a.nod_id, a.dir_id, a.vids_id, a.kato_id, a.depo_id, a.dep_id
from (
	select 	ind.hcode_id, ind.hcode_name, ind.hcode_unit_name, ind.org_id, ind.dor_kod, 4 as date_type_id, ind.metric_type_id, ind.cargo_type_id, 
			5 as val_type_id, ind.unit_id, ind.dt, 
			case
					...
					then 
						...
						+ ind5.value)/		
						(case when ind.value is not null then 1 else 0 end + case when ind2.value is not null then 1 else 0 end + 
						case when ind3.value is not null then 1 else 0 end + case when ind4.value is not null then 1 else 0 end + 
						case when ind5.value is not null then 1 else 0 end))::numeric(20,8) 
					else
						((ind.value*dte.month_length + ind2.value*dte2.month_length + ind3.value*dte3.month_length 
						...
						+ (case when ind2.value is not null then 1 else 0 end)*dte2.month_length 
						+ (case when ind3.value is not null then 1 else 0 end)*dte3.month_length 
						+ (case when ind4.value is not null then 1 else 0 end)*dte4.month_length
						+ (case when ind5.value is not null then 1 else 0 end)*dte5.month_length))::numeric(20,8) 
					end as value,  
			ind.ss, ind.duch_id, ind.nod_id, ind.dir_id, ind.vids_id, ind.kato_id, ind.depo_id, ind.dep_id
		from 	( 	select * from dm_stg.calc_src_indicators_t 
...
and ind.dep_id = ind5.dep_id
	...
	join dm_stg.d_date_t dte3 on dte3.dt = ind3.dt
	join dm_stg.d_date_t dte4 on dte4.dt = ind4.dt
	join dm_stg.d_date_t dte5 on dte5.dt = ind5.dt
	where dte.%s
	) a
left join ( -- Секция выявления более приоритетных дублей
	select * 
...
# -- Сверка кол-ва записей 
query_2_2_10_11 = ("""select 'tgt', count(0) 
	from dm_rep.dm_all_indicators_v clc
	join (select distinct id, group_type from dm_stg.d_hcode_t) hcd
		on clc.hcode_id = hcd.id
where 	calc_rule = 'growplan'
...
# -- Сравнение всех записей в таблицах
query_2_2_10_12 = ("""select 	a.hcode_id, a.hcode_name, a.hcode_unit_name, a.org_id, a.dor_kod, a.date_type_id, a.metric_type_id, a.cargo_type_id, 
		a.val_type_id, a.unit_id, a.dt, a.value, a.ss, a.duch_id, a.nod_id, a.dir_id, a.vids_id, a.kato_id, a.depo_id, a.dep_id
	from (
		select 	ind.hcode_id, ind.hcode_name, ind.hcode_unit_name, ind.org_id, ind.dor_kod, 4 as date_type_id, ind.metric_type_id, ind.cargo_type_id, 
				5 as val_type_id, ind.unit_id, ind.dt, 
				case
					...
					then 
						((ind.value + ind2.value + ind3.value + ind4.value 
						+ ind5.value + ind6.value)/		
						(case when ind.value is not null then 1 else 0 end + case when ind2.value is not null then 1 else 0 end + 
						case when ind3.value is not null then 1 else 0 end + case when ind4.value is not null then 1 else 0 end + 
						case when ind5.value is not null then 1 else 0 end + case when ind6.value is not null then 1 else 0 end))::numeric(20,8) 
					else
						((ind.value*dte.month_length + ind2.value*dte2.month_length + ind3.value*dte3.month_length 
						...
						+ (case when ind3.value is not null then 1 else 0 end)*dte3.month_length 
						+ (case when ind4.value is not null then 1 else 0 end)*dte4.month_length
						+ (case when ind5.value is not null then 1 else 0 end)*dte5.month_length
						+ (case when ind6.value is not null then 1 else 0 end)*dte6.month_length))::numeric(20,8) 
					end as value,  
				ind.ss, ind.duch_id, ind.nod_id, ind.dir_id, ind.vids_id, ind.kato_id, ind.depo_id, ind.dep_id
			from 	( 	select * from dm_stg.calc_src_indicators_t 
...
and ind.dep_id = ind6.dep_id
	...
	join dm_stg.d_date_t dte4 on dte4.dt = ind4.dt
	join dm_stg.d_date_t dte5 on dte5.dt = ind5.dt
	join dm_stg.d_date_t dte6 on dte6.dt = ind6.dt
	where ind.%s
	) a
left join ( -- Секция выявления более приоритетных дублей
	select * from dm_stg.calc_src_indicators_t ind 
...
# -- Сверка кол-ва записей 
query_2_2_10_13 = ("""select 'tgt', count(0) from dm_rep.dm_all_indicators_v clc
join (select distinct id, group_type from dm_stg.d_hcode_t) hcd
	on clc.hcode_id = hcd.id
where 	calc_rule = 'growplan'
	and val_type_id = 5
...
# -- Сравнение всех записей в таблицах
query_2_2_10_14 = ("""select 	a.hcode_id, a.hcode_name, a.hcode_unit_name, a.org_id, a.dor_kod, a.date_type_id, a.metric_type_id, a.cargo_type_id, 
		a.val_type_id, a.unit_id, a.dt, a.value, a.ss, a.duch_id, a.nod_id, a.dir_id, a.vids_id, a.kato_id, a.depo_id, a.dep_id
	from (
		select 	ind.hcode_id, ind.hcode_name, ind.hcode_unit_name, ind.org_id, ind.dor_kod, 4 as date_type_id, ind.metric_type_id, ind.cargo_type_id, 
				5 as val_type_id, ind.unit_id, ind.dt, 
				case
					...
					then 
						((ind.value + ind2.value + ind3.value + ind4.value 
						+ ind5.value + ind6.value + ind7.value)/		
						(case when ind.value is not null then 1 else 0 end + case when ind2.value is not null then 1 else 0 end + 
						case when ind3.value is not null then 1 else 0 end + case when ind4.value is not null then 1 else 0 end + 
						case when ind5.value is not null then 1 else 0 end + case when ind6.value is not null then 1 else 0 end + 
						case when ind7.value is not null then 1 else 0 end))::numeric(20,12) 
					else
						((ind.value*dte.month_length + ind2.value*dte2.month_length + ind3.value*dte3.month_length 
						+ ind4.value*dte4.month_length + ind5.value*dte5.month_length + ind6.value*dte6.month_length 
						...
						+ (case when ind4.value is not null then 1 else 0 end)*dte4.month_length
						+ (case when ind5.value is not null then 1 else 0 end)*dte5.month_length
						+ (case when ind6.value is not null then 1 else 0 end)*dte6.month_length
						+ (case when ind7.value is not null then 1 else 0 end)*dte7.month_length))::numeric(20,12) 
					end as value,  
				ind.ss, ind.duch_id, ind.nod_id, ind.dir_id, ind.vids_id, ind.kato_id, ind.depo_id, ind.dep_id
			from 	( 	select * from dm_stg.calc_src_indicators_t 
...
and ind.dep_id = ind7.dep_id
	...
	join dm_stg.d_date_t dte5 on dte5.dt = ind5.dt
	join dm_stg.d_date_t dte6 on dte6.dt = ind6.dt
	join dm_stg.d_date_t dte7 on dte7.dt = ind7.dt
	where ind.%s
	) a
left join ( -- Секция выявления более приоритетных дублей
	select  *
...
# -- Сверка кол-ва записей 
query_2_2_10_15 = ("""select 'tgt', count(0)
		from dm_rep.dm_all_indicators_v clc
		join (select distinct id, group_type from dm_stg.d_hcode_t) hcd
			on clc.hcode_id = hcd.id
where 	calc_rule = 'growplan'
...
# -- Сравнение всех записей в таблицах
query_2_2_10_16 = ("""select 	a.hcode_id, a.hcode_name, a.hcode_unit_name, a.org_id, a.dor_kod, a.date_type_id, a.metric_type_id, a.cargo_type_id, 
		a.val_type_id, a.unit_id, a.dt, a.value, a.ss, a.duch_id, a.nod_id, a.dir_id, a.vids_id, a.kato_id, a.depo_id, a.dep_id
from (
select 	ind.hcode_id, ind.hcode_name, ind.hcode_unit_name, ind.org_id, ind.dor_kod, 4 as date_type_id, ind.metric_type_id, ind.cargo_type_id, 
		5 as val_type_id, ind.unit_id, ind.dt, 
		case
					...
					then 
						...
						case when ind3.value is not null then 1 else 0 end + case when ind4.value is not null then 1 else 0 end + 
						case when ind5.value is not null then 1 else 0 end + case when ind6.value is not null then 1 else 0 end + 
						case when ind7.value is not null then 1 else 0 end + case when ind8.value is not null then 1 else 0 end))::numeric(20,12) 
					else
						((ind.value*dte.month_length + ind2.value*dte2.month_length + ind3.value*dte3.month_length 
						+ ind4.value*dte4.month_length + ind5.value*dte5.month_length + ind6.value*dte6.month_length 
						...
						+ (case when ind5.value is not null then 1 else 0 end)*dte5.month_length
						+ (case when ind6.value is not null then 1 else 0 end)*dte6.month_length
						+ (case when ind7.value is not null then 1 else 0 end)*dte7.month_length
						+ (case when ind8.value is not null then 1 else 0 end)*dte8.month_length))::numeric(20,12) 
					end as value,   
		ind.ss, ind.duch_id, ind.nod_id, ind.dir_id, ind.vids_id, ind.kato_id, ind.depo_id, ind.dep_id
	from 	( 	select * from dm_stg.calc_src_indicators_t 
...
and ind.dep_id = ind8.dep_id
	...
	join dm_stg.d_date_t dte6 on dte6.dt = ind6.dt
	join dm_stg.d_date_t dte7 on dte7.dt = ind7.dt
	join dm_stg.d_date_t dte8 on dte8.dt = ind8.dt
	where dte.%s
	) a
left join ( -- Секция выявления более приоритетных дублей
	select  *
...
# -- Сверка кол-ва записей 
query_2_2_10_17 = ("""select 'tgt', count(0)
	from dm_rep.dm_all_indicators_v clc
	join (select distinct id, group_type from dm_stg.d_hcode_t) hcd
		on clc.hcode_id = hcd.id
where 	calc_rule = 'growplan'
...
# -- Сравнение всех записей в таблицах
query_2_2_10_18 = ("""select 	a.hcode_id, a.hcode_name, a.hcode_unit_name, a.org_id, a.dor_kod, a.date_type_id, a.metric_type_id, a.cargo_type_id, 
		a.val_type_id, a.unit_id, a.dt, a.value, a.ss, a.duch_id, a.nod_id, a.dir_id, a.vids_id, a.kato_id, a.depo_id, a.dep_id
from (
select 	ind.hcode_id, ind.hcode_name, ind.hcode_unit_name, ind.org_id, ind.dor_kod, 4 as date_type_id, ind.metric_type_id, ind.cargo_type_id, 
		5 as val_type_id, ind.unit_id, ind.dt, 
		case
					...
					then 
						...
						case when ind3.value is not null then 1 else 0 end + case when ind4.value is not null then 1 else 0 end + 
						case when ind5.value is not null then 1 else 0 end + case when ind6.value is not null then 1 else 0 end + 
						case when ind7.value is not null then 1 else 0 end + case when ind8.value is not null then 1 else 0 end + 
						case when ind9.value is not null then 1 else 0 end))::numeric(20,12) 
					else
						((ind.value*dte.month_length + ind2.value*dte2.month_length + ind3.value*dte3.month_length 
						+ ind4.value*dte4.month_length + ind5.value*dte5.month_length + ind6.value*dte6.month_length 
						...
						+ (case when ind6.value is not null then 1 else 0 end)*dte6.month_length
						+ (case when ind7.value is not null then 1 else 0 end)*dte7.month_length
						+ (case when ind8.value is not null then 1 else 0 end)*dte8.month_length
						+ (case when ind9.value is not null then 1 else 0 end)*dte9.month_length))::numeric(20,12) 
					end as value,  
			ind.ss, ind.duch_id, ind.nod_id, ind.dir_id, ind.vids_id, ind.kato_id, ind.depo_id, ind.dep_id
	from 	( 	select * from dm_stg.calc_src_indicators_t 
...
and ind.dep_id = ind9.dep_id
	...
	join dm_stg.d_date_t dte7 on dte7.dt = ind7.dt
	join dm_stg.d_date_t dte8 on dte8.dt = ind8.dt
	join dm_stg.d_date_t dte9 on dte9.dt = ind9.dt
	where dte.%s
	) a
left join ( -- Секция выявления более приоритетных дублей
	select  *
...
# -- Сверка кол-ва записей 
query_2_2_10_19 = ("""select 'tgt', count(0) 
	from dm_rep.dm_all_indicators_v clc
	join (select distinct id, group_type from dm_stg.d_hcode_t) hcd
		on clc.hcode_id = hcd.id
where 	calc_rule = 'growplan'
...
# -- Сравнение всех записей в таблицах
query_2_2_10_20 = ("""select 	a.hcode_id, a.hcode_name, a.hcode_unit_name, a.org_id, a.dor_kod, a.date_type_id, a.metric_type_id, a.cargo_type_id, 
		a.val_type_id, a.unit_id, a.dt, a.value, a.ss, a.duch_id, a.nod_id, a.dir_id, a.vids_id, a.kato_id, a.depo_id, a.dep_id
	from (
		select 	ind.hcode_id, ind.hcode_name, ind.hcode_unit_name, ind.org_id, ind.dor_kod, 4 as date_type_id, ind.metric_type_id, ind.cargo_type_id, 
				5 as val_type_id, ind.unit_id, ind.dt, 
				case
					...
					then 
						((ind.value + ind2.value + ind3.value + ind4.value 
						+ ind5.value + ind6.value + ind7.value + ind8.value 
						+ ind9.value + ind10.value)/		
						(case when ind.value is not null then 1 else 0 end + case when ind2.value is not null then 1 else 0 end + 
						case when ind3.value is not null then 1 else 0 end + case when ind4.value is not null then 1 else 0 end + 
						case when ind5.value is not null then 1 else 0 end + case when ind6.value is not null then 1 else 0 end + 
						case when ind7.value is not null then 1 else 0 end + case when ind8.value is not null then 1 else 0 end + 
						case when ind9.value is not null then 1 else 0 end + case when ind10.value is not null then 1 else 0 end))::numeric(20,12) 
					when 
						ind.hcode_id = '00025'
					then
						((ind.value*dte.month_length + ind2.value*dte2.month_length + ind3.value*dte3.month_length 
						...
						+ (case when ind7.value is not null then 1 else 0 end)*dte7.month_length
						+ (case when ind8.value is not null then 1 else 0 end)*dte8.month_length
						+ (case when ind9.value is not null then 1 else 0 end)*dte9.month_length
						+ (case when ind10.value is not null then 1 else 0 end)*dte10.month_length))::numeric(20,12) 
					else
						((ind.value*dte.month_length + ind2.value*dte2.month_length + ind3.value*dte3.month_length 
						+ ind4.value*dte4.month_length + ind5.value*dte5.month_length + ind6.value*dte6.month_length 
						+ ind7.value*dte7.month_length + ind8.value*dte8.month_length + ind9.value*dte9.month_length 
...
and ind.dep_id = ind10.dep_id
	...
	join dm_stg.d_date_t dte8 on dte8.dt = ind8.dt
	join dm_stg.d_date_t dte9 on dte9.dt = ind9.dt
	join dm_stg.d_date_t dte10 on dte10.dt = ind10.dt
	where dte.%s
	) a
left join ( -- Секция выявления более приоритетных дублей
	select  *
...
# -- Сверка кол-ва записей 
query_2_2_10_21 = ("""select 'tgt', count(0)
		from dm_rep.dm_all_indicators_v clc
		join (select distinct id, group_type from dm_stg.d_hcode_t) hcd
			on clc.hcode_id = hcd.id
where 	calc_rule = 'growplan'
...
# -- Сравнение всех записей в таблицах
query_2_2_10_22 = ("""select 	a.hcode_id, a.hcode_name, a.hcode_unit_name, a.org_id, a.dor_kod, a.date_type_id, a.metric_type_id, a.cargo_type_id, 
		a.val_type_id, a.unit_id, a.dt, a.value, a.ss, a.duch_id, a.nod_id, a.dir_id, a.vids_id, a.kato_id, a.depo_id, a.dep_id
	from (
		select 	ind.hcode_id, ind.hcode_name, ind.hcode_unit_name, ind.org_id, ind.dor_kod, 4 as date_type_id, ind.metric_type_id, ind.cargo_type_id, 
				5 as val_type_id, ind.unit_id, ind.dt, 
				case
					...
					then 
						((ind.value + ind2.value + ind3.value + ind4.value 
						+ ind5.value + ind6.value + ind7.value + ind8.value 
						+ ind9.value + ind10.value + ind11.value)/		
						(case when ind.value is not null then 1 else 0 end + case when ind2.value is not null then 1 else 0 end + 
						case when ind3.value is not null then 1 else 0 end + case when ind4.value is not null then 1 else 0 end + 
						case when ind5.value is not null then 1 else 0 end + case when ind6.value is not null then 1 else 0 end + 
						case when ind7.value is not null then 1 else 0 end + case when ind8.value is not null then 1 else 0 end + 
						case when ind9.value is not null then 1 else 0 end + case when ind10.value is not null then 1 else 0 end + 
						case when ind11.value is not null then 1 else 0 end))::numeric(20,12) 
					when 
						ind.hcode_id = '00025'
					then
						((ind.value*dte.month_length + ind2.value*dte2.month_length + ind3.value*dte3.month_length 
						...
						+ (case when ind8.value is not null then 1 else 0 end)*dte8.month_length
						+ (case when ind9.value is not null then 1 else 0 end)*dte9.month_length
						+ (case when ind10.value is not null then 1 else 0 end)*dte10.month_length
						+ (case when ind11.value is not null then 1 else 0 end)*dte11.month_length))::numeric(20,12) 
					else
						((ind.value*dte.month_length + ind2.value*dte2.month_length + ind3.value*dte3.month_length 
						+ ind4.value*dte4.month_length + ind5.value*dte5.month_length + ind6.value*dte6.month_length 
						+ ind7.value*dte7.month_length + ind8.value*dte8.month_length + ind9.value*dte9.month_length 
...
and ind.dep_id = ind11.dep_id
	...
	join dm_stg.d_date_t dte9 on dte9.dt = ind9.dt
	join dm_stg.d_date_t dte10 on dte10.dt = ind10.dt
	join dm_stg.d_date_t dte11 on dte11.dt = ind11.dt
	where dte.%s
	) a
left join ( -- Секция выявления более приоритетных дублей
	select  *
...
# -- Сверка кол-ва записей 
query_2_2_10_23 = ("""select 'tgt', count(0)
		from dm_rep.dm_all_indicators_v clc
		join (select distinct id, group_type from dm_stg.d_hcode_t) hcd
			on clc.hcode_id = hcd.id
where 	calc_rule = 'growplan'
...
# -- Сравнение всех записей в таблицах
query_2_2_10_24 = ("""select 	a.hcode_id, a.hcode_name, a.hcode_unit_name, a.org_id, a.dor_kod, a.date_type_id, a.metric_type_id, a.cargo_type_id, 
		a.val_type_id, a.unit_id, a.dt, a.value, a.ss, a.duch_id, a.nod_id, a.dir_id, a.vids_id, a.kato_id, a.depo_id, a.dep_id
from (
select 	ind.hcode_id, ind.hcode_name, ind.hcode_unit_name, ind.org_id, ind.dor_kod, 4 as date_type_id, ind.metric_type_id, ind.cargo_type_id, 
		5 as val_type_id, ind.unit_id, ind.dt, 
		case
			...
			then 
				((ind.value + ind2.value + ind3.value + ind4.value 
				+ ind5.value + ind6.value + ind7.value + ind8.value 
				+ ind9.value + ind10.value + ind11.value + ind12.value)/		
				(case when ind.value is not null then 1 else 0 end + case when ind2.value is not null then 1 else 0 end + 
				case when ind3.value is not null then 1 else 0 end + case when ind4.value is not null then 1 else 0 end + 
				case when ind5.value is not null then 1 else 0 end + case when ind6.value is not null then 1 else 0 end + 
				case when ind7.value is not null then 1 else 0 end + case when ind8.value is not null then 1 else 0 end + 
				case when ind9.value is not null then 1 else 0 end + case when ind10.value is not null then 1 else 0 end + 
				case when ind11.value is not null then 1 else 0 end + case when ind12.value is not null then 1 else 0 end))::numeric(20,12) 
			when 
						ind.hcode_id = '00025'
					then
						((ind.value*dte.month_length + ind2.value*dte2.month_length + ind3.value*dte3.month_length 
						...
						+ (case when ind9.value is not null then 1 else 0 end)*dte9.month_length
						+ (case when ind10.value is not null then 1 else 0 end)*dte10.month_length
						+ (case when ind11.value is not null then 1 else 0 end)*dte11.month_length
						+ (case when ind12.value is not null then 1 else 0 end)*dte12.month_length))::numeric(20,12) 
			else
				((ind.value*dte.month_length + ind2.value*dte2.month_length + ind3.value*dte3.month_length 
				+ ind4.value*dte4.month_length + ind5.value*dte5.month_length + ind6.value*dte6.month_length 
				+ ind7.value*dte7.month_length + ind8.value*dte8.month_length + ind9.value*dte9.month_length 
...
and ind.dep_id = ind12.dep_id
	...
	join dm_stg.d_date_t dte10 on dte10.dt = ind10.dt
	join dm_stg.d_date_t dte11 on dte11.dt = ind11.dt
	join dm_stg.d_date_t dte12 on dte12.dt = ind12.dt
	where dte.%s
	) a
left join ( -- Секция выявления более приоритетных дублей
	select  *
...
QUERYS_2_10 = [v for v in locals() if v.startswith('query')]

QUERYS_2_10_EQUAL = [n for n in QUERYS_2_10 if QUERYS_2_10.index(n) % 2 == 0]
QUERYS_2_10_EMPTY = [n for n in QUERYS_2_10 if QUERYS_2_10.index(n) % 2 != 0]