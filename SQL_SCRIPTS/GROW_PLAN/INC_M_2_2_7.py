# -----------------------------------------------------------------------
# ---- 2.2.7 Нарастающий итог по месяцам с начала квартала. Базовый. ----
# -----------------------------------------------------------------------


# /*
# sum(CQ where val_type=7 and date_type=4)	
# Берем значения за прошедшие месяцы с начала квартала до "текущего"
#  */
...
query_2_2_7_1 = ("""select 'tgt', count(0)
...
query_2_2_7_2 = ("""select 	a.hcode_id, a.hcode_name, a.hcode_unit_name, a.org_id, a.dor_kod, a.date_type_id, a.metric_type_id, a.cargo_type_id, 
...
# Сверка кол-ва записей
query_2_2_7_3 = ("""select 'tgt', count(0)
		from dm_rep.dm_all_indicators_v clc
		join (select distinct id, group_type from dm_stg.d_hcode_t) hcd
			on clc.hcode_id = hcd.id
		join dm_stg.d_date_t dte
...
# Сравнение всех записей в таблицах
query_2_2_7_4 = ("""select 	a.hcode_id, a.hcode_name, a.hcode_unit_name, a.org_id, a.dor_kod, a.date_type_id, a.metric_type_id, a.cargo_type_id, 
		a.val_type_id, a.unit_id, a.dt, a.value, a.ss, a.duch_id, a.nod_id, a.dir_id, a.vids_id, a.kato_id, a.depo_id, a.dep_id
from (
select 	ind.hcode_id, ind.hcode_name, ind.hcode_unit_name, ind.org_id, ind.dor_kod, 4 as date_type_id, ind.metric_type_id, 
...
# Сверка кол-ва записей
query_2_2_7_5 = ("""select 'tgt', count(0)
		from dm_rep.dm_all_indicators_v clc
		join (select distinct id, group_type from dm_stg.d_hcode_t) hcd
			on clc.hcode_id = hcd.id
		join dm_stg.d_date_t dte
...
# Сравнение всех записей в таблицах
query_2_2_7_6 = ("""select 	a.hcode_id, a.hcode_name, a.hcode_unit_name, a.org_id, a.dor_kod, a.date_type_id, a.metric_type_id, a.cargo_type_id, 
		a.val_type_id, a.unit_id, a.dt, a.value, a.ss, a.duch_id, a.nod_id, a.dir_id, a.vids_id, a.kato_id, a.depo_id, a.dep_id
from (
select 	ind.hcode_id, ind.hcode_name, ind.hcode_unit_name, ind.org_id, ind.dor_kod, 4 as date_type_id, ind.metric_type_id, 
		ind.cargo_type_id, 9 as val_type_id, ind.unit_id, dte.dt, (ind.value + ind2.value 
		+ ind3.value)::numeric(20,10) as value, ind.ss, ind.duch_id, ind.nod_id, ind.dir_id, ind.vids_id, ind.kato_id, ind.depo_id, ind.dep_id
from ( 	select * from dm_stg.calc_src_indicators_t 
		where val_type_id = 7 and date_type_id = 4 and metric_type_id in (12,2,18)
	 ) ind 
		join dm_stg.d_date_t dte
...
QUERYS_2_7 = [v for v in locals() if v.startswith('query')]

QUERYS_2_7_EQUAL = [n for n in QUERYS_2_7 if QUERYS_2_7.index(n) % 2 == 0]
QUERYS_2_7_EMPTY = [n for n in QUERYS_2_7 if QUERYS_2_7.index(n) % 2 != 0]