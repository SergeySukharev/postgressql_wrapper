# -------------------------------------------------------------------------
# ---- 2.2.6 Нарастающий итог по суткам с начала года. Среднесуточный. ----
# -------------------------------------------------------------------------


# /*
# 00045 - 00054
# sum(CY where val_type=7 and date_type=4)/количество месяцев с данными	
# Если нет строчки 1 12/2 3 (целевые значения), берем значения за прошедшие месяцы до "текущего" и размножаем его на все дни этого месяца
# НЕ 00045 - 00054
...
query_2_2_6_1 = ("""select 'tgt', count(0)
...
query_2_2_6_2 = ("""select 	a.hcode_id, a.hcode_name, a.hcode_unit_name, a.org_id, a.dor_kod, a.date_type_id, a.metric_type_id, a.cargo_type_id, 
...
select 	ind.hcode_id, ind.hcode_name, ind.hcode_unit_name, ind.org_id, ind.dor_kod, 3 as date_type_id, ind.metric_type_id, 
		ind.cargo_type_id, 1 as val_type_id, ind.unit_id, dte.dt, ind.value::numeric(20,12) as value, 
...
# Сверка кол-ва записей
query_2_2_6_3 = ("""select 'tgt', count(0) 
		from dm_rep.dm_all_indicators_v clc
		join (select distinct id, group_type from dm_stg.d_hcode_t) hcd
			on clc.hcode_id = hcd.id
where 	calc_rule = 'growplan'
...
# Сравнение всех записей в таблицах
query_2_2_6_4 = ("""select 	a.hcode_id, a.hcode_name, a.hcode_unit_name, a.org_id, a.dor_kod, a.date_type_id, a.metric_type_id, a.cargo_type_id, 
		a.val_type_id, a.unit_id, a.dt, a.value, a.ss, a.duch_id, a.nod_id, a.dir_id, a.vids_id, a.kato_id, a.depo_id, a.dep_id
from (
select 	ind.hcode_id, ind.hcode_name, ind.hcode_unit_name, ind.org_id, ind.dor_kod, 3 as date_type_id, ind.metric_type_id, 
		...
		case 
			...
			then 
				...
				+ (case when ind2.value is not null then 1 else 0 end)*dte2.month_length))::numeric(20,6) end as value, 
		ind.ss, ind.duch_id, ind.nod_id, ind.dir_id, ind.vids_id, ind.kato_id, ind.depo_id, ind.dep_id
from ( 	select * from dm_stg.calc_src_indicators_t 
...
join dm_stg.d_date_t dte2 on ind2.dt = dte2.dt
where dte.%s
) a
left join (-- Секция выявления более приоритетных дублей
	select  *
...
# Сверка кол-ва записей
query_2_2_6_5 = ("""select 'tgt', count(0)
		from dm_rep.dm_all_indicators_v clc
		join (select distinct id, group_type from dm_stg.d_hcode_t) hcd
			on clc.hcode_id = hcd.id
where 	calc_rule = 'growplan'
...
# Сравнение всех записей в таблицах
query_2_2_6_6 = ("""select 	a.hcode_id, a.hcode_name, a.hcode_unit_name, a.org_id, a.dor_kod, a.date_type_id, a.metric_type_id, a.cargo_type_id, 
		a.val_type_id, a.unit_id, a.dt, a.value, a.ss, a.duch_id, a.nod_id, a.dir_id, a.vids_id, a.kato_id, a.depo_id, a.dep_id
from (
select 	ind.hcode_id, ind.hcode_name, ind.hcode_unit_name, ind.org_id, ind.dor_kod, 3 as date_type_id, ind.metric_type_id, 
		ind.cargo_type_id, 1 as val_type_id, ind.unit_id, dte.dt, 
		case
			...
			then 
				...
				+ (case when ind3.value is not null then 1 else 0 end)*dte3.month_length))::numeric(20,6) 
			end as value, 
		ind.ss, ind.duch_id, ind.nod_id, ind.dir_id, ind.vids_id, ind.kato_id, ind.depo_id, ind.dep_id
from ( 	select * from dm_stg.calc_src_indicators_t 
...
and ind.dep_id = ind3.dep_id
	...
	join dm_stg.d_date_t dte2 on ind2.dt = dte2.dt
	join dm_stg.d_date_t dte3 on ind3.dt = dte3.dt
	where dte.%s
	) a
left join (-- Секция выявления более приоритетных дублей
	select  *
...
# Сверка кол-ва записей
query_2_2_6_7 = ("""select 'tgt', count(0)
		from dm_rep.dm_all_indicators_v clc
		join (select distinct id, group_type from dm_stg.d_hcode_t) hcd
			on clc.hcode_id = hcd.id
where 	calc_rule = 'growplan'
...
query_2_2_6_8 = ("""select 	a.hcode_id, a.hcode_name, a.hcode_unit_name, a.org_id, a.dor_kod, a.date_type_id, a.metric_type_id, a.cargo_type_id, 
		a.val_type_id, a.unit_id, a.dt, a.value, a.ss, a.duch_id, a.nod_id, a.dir_id, a.vids_id, a.kato_id, a.depo_id, a.dep_id
from (
select 	ind.hcode_id, ind.hcode_name, ind.hcode_unit_name, ind.org_id, ind.dor_kod, 3 as date_type_id, ind.metric_type_id, 
		ind.cargo_type_id, 1 as val_type_id, ind.unit_id, dte.dt, 
		case
			...
			then 
				...
				(case when ind.value is not null then 1 else 0 end + case when ind2.value is not null then 1 else 0 end 
				+ case when ind3.value is not null then 1 else 0 end + case when ind4.value is not null then 1 else 0 end))::numeric(20,6) 
			else
				((ind.value*dte.month_length + ind2.value*dte2.month_length + ind3.value*dte3.month_length
				...
				+ (case when ind4.value is not null then 1 else 0 end)*dte4.month_length))::numeric(20,6) 
			end as value,
		ind.ss, ind.duch_id, ind.nod_id, ind.dir_id, ind.vids_id, ind.kato_id, ind.depo_id, ind.dep_id
from ( 	select * from dm_stg.calc_src_indicators_t 
...
and ind.dep_id = ind4.dep_id
	...
	join dm_stg.d_date_t dte2 on ind2.dt = dte2.dt
	join dm_stg.d_date_t dte3 on ind3.dt = dte3.dt
	join dm_stg.d_date_t dte4 on ind4.dt = dte4.dt
	where dte.%s
	) a
left join (-- Секция выявления более приоритетных дублей
	select  *
...
# Сверка кол-ва записей
query_2_2_6_9 = ("""select 'tgt', count(0)
		from dm_rep.dm_all_indicators_v clc
		join (select distinct id, group_type from dm_stg.d_hcode_t) hcd
			on clc.hcode_id = hcd.id
where 	calc_rule = 'growplan'
...
query_2_2_6_10 = ("""select 	a.hcode_id, a.hcode_name, a.hcode_unit_name, a.org_id, a.dor_kod, a.date_type_id, a.metric_type_id, a.cargo_type_id, 
		a.val_type_id, a.unit_id, a.dt, a.value, a.ss, a.duch_id, a.nod_id, a.dir_id, a.vids_id, a.kato_id, a.depo_id, a.dep_id
from (
select 	ind.hcode_id, ind.hcode_name, ind.hcode_unit_name, ind.org_id, ind.dor_kod, 3 as date_type_id, ind.metric_type_id, ind.cargo_type_id, 
		...
		case
			when
				ind.hcode_id between '00045' and '00054' 
			then 
				((ind.value + ind2.value + ind3.value + ind4.value + ind5.value)/
				(case when ind.value is not null then 1 else 0 end + case when ind2.value is not null then 1 else 0 end 
				+ case when ind3.value is not null then 1 else 0 end + case when ind4.value is not null then 1 else 0 end
				+ case when ind5.value is not null then 1 else 0 end))::numeric(20,6) 
			else
				((ind.value*dte.month_length + ind2.value*dte2.month_length + ind3.value*dte3.month_length
				...
				+ (case when ind5.value is not null then 1 else 0 end)*dte5.month_length))::numeric(20,6) 
			end as value,
		ind.ss, ind.duch_id, ind.nod_id, ind.dir_id, ind.vids_id, ind.kato_id, ind.depo_id, ind.dep_id
from ( 	select * from dm_stg.calc_src_indicators_t 
...
and ind.dep_id = ind5.dep_id
	...
	join dm_stg.d_date_t dte3 on ind3.dt = dte3.dt
	join dm_stg.d_date_t dte4 on ind4.dt = dte4.dt
	join dm_stg.d_date_t dte5 on ind5.dt = dte5.dt
	where dte.%s
	) a
left join (-- Секция выявления более приоритетных дублей
	select  *
...
# Сверка кол-ва записей
query_2_2_6_11 = ("""select 'tgt', count(0)
		from dm_rep.dm_all_indicators_v clc
		join (select distinct id, group_type from dm_stg.d_hcode_t) hcd
			on clc.hcode_id = hcd.id
where 	calc_rule = 'growplan'
...
query_2_2_6_12 = ("""select 	a.hcode_id, a.hcode_name, a.hcode_unit_name, a.org_id, a.dor_kod, a.date_type_id, a.metric_type_id, a.cargo_type_id, 
		a.val_type_id, a.unit_id, a.dt, a.value, a.ss, a.duch_id, a.nod_id, a.dir_id, a.vids_id, a.kato_id, a.depo_id, a.dep_id
from (
select 	ind.hcode_id, ind.hcode_name, ind.hcode_unit_name, ind.org_id, ind.dor_kod, 3 as date_type_id, ind.metric_type_id, ind.cargo_type_id, 
		1 as val_type_id, ind.unit_id, dte.dt, 
		case
			...
			then 
				...
				(case when ind.value is not null then 1 else 0 end + case when ind2.value is not null then 1 else 0 end 
				+ case when ind3.value is not null then 1 else 0 end + case when ind4.value is not null then 1 else 0 end
				+ case when ind5.value is not null then 1 else 0 end + case when ind6.value is not null then 1 else 0 end))::numeric(20,6) 
			else
				((ind.value*dte.month_length + ind2.value*dte2.month_length + ind3.value*dte3.month_length
				...
				+ (case when ind6.value is not null then 1 else 0 end)*dte6.month_length))::numeric(20,6) 
			end as value,
		ind.ss, ind.duch_id, ind.nod_id, ind.dir_id, ind.vids_id, ind.kato_id, ind.depo_id, ind.dep_id
from ( 	select * from dm_stg.calc_src_indicators_t 
...
and ind.dep_id = ind6.dep_id
	...
	join dm_stg.d_date_t dte4 on ind4.dt = dte4.dt
	join dm_stg.d_date_t dte5 on ind5.dt = dte5.dt
	join dm_stg.d_date_t dte6 on ind6.dt = dte6.dt
	where dte.%s
	) a
left join (-- Секция выявления более приоритетных дублей
	select  *
...
# Сверка кол-ва записей
query_2_2_6_13 = ("""select 'tgt', count(0)
		from dm_rep.dm_all_indicators_v clc
		join (select distinct id, group_type from dm_stg.d_hcode_t) hcd
			on clc.hcode_id = hcd.id
where 	calc_rule = 'growplan'
...
query_2_2_6_14 = ("""select 	a.hcode_id, a.hcode_name, a.hcode_unit_name, a.org_id, a.dor_kod, a.date_type_id, a.metric_type_id, a.cargo_type_id, 
		a.val_type_id, a.unit_id, a.dt, a.value, a.ss, a.duch_id, a.nod_id, a.dir_id, a.vids_id, a.kato_id, a.depo_id, a.dep_id
from (
select 	ind.hcode_id, ind.hcode_name, ind.hcode_unit_name, ind.org_id, ind.dor_kod, 3 as date_type_id, ind.metric_type_id, ind.cargo_type_id, 
		1 as val_type_id, ind.unit_id, dte.dt, 
		case
			...
			then 
				...
				+ (case when ind7.value is not null then 1 else 0 end)*dte7.month_length))::numeric(20,6) 
			end as value,
		ind.ss, ind.duch_id, ind.nod_id, ind.dir_id, ind.vids_id, ind.kato_id, ind.depo_id, ind.dep_id
from ( 	select * from dm_stg.calc_src_indicators_t 
...
and ind.dep_id = ind7.dep_id
	...
	join dm_stg.d_date_t dte5 on ind5.dt = dte5.dt
	join dm_stg.d_date_t dte6 on ind6.dt = dte6.dt
	join dm_stg.d_date_t dte7 on ind7.dt = dte7.dt
	where dte.%s
	) a
left join ( -- Секция выявления более приоритетных дублей
	select  *
...
# Сверка кол-ва записей
query_2_2_6_15 = ("""select 'tgt', count(0)
		from dm_rep.dm_all_indicators_v clc
		join (select distinct id, group_type from dm_stg.d_hcode_t) hcd
			on clc.hcode_id = hcd.id
where 	calc_rule = 'growplan'
...
query_2_2_6_16 = ("""select 	a.hcode_id, a.hcode_name, a.hcode_unit_name, a.org_id, a.dor_kod, a.date_type_id, a.metric_type_id, a.cargo_type_id, 
		a.val_type_id, a.unit_id, a.dt, a.value, a.ss, a.duch_id, a.nod_id, a.dir_id, a.vids_id, a.kato_id, a.depo_id, a.dep_id
from (
select 	ind.hcode_id, ind.hcode_name, ind.hcode_unit_name, ind.org_id, ind.dor_kod, 3 as date_type_id, ind.metric_type_id, ind.cargo_type_id, 
		1 as val_type_id, ind.unit_id, dte.dt, 
		case
			...
			then 
				((ind.value + ind2.value + ind3.value + ind4.value + ind5.value 
				+ ind6.value + ind7.value + ind8.value)/
				(case when ind.value is not null then 1 else 0 end + case when ind2.value is not null then 1 else 0 end 
				+ case when ind3.value is not null then 1 else 0 end + case when ind4.value is not null then 1 else 0 end
				+ case when ind5.value is not null then 1 else 0 end + case when ind6.value is not null then 1 else 0 end
				+ case when ind7.value is not null then 1 else 0 end + case when ind8.value is not null then 1 else 0 end))::numeric(20,6) 
			else
				((ind.value*dte.month_length + ind2.value*dte2.month_length + ind3.value*dte3.month_length
				 + ind4.value*dte4.month_length + ind5.value*dte5.month_length + ind6.value*dte6.month_length
				...
				+ (case when ind8.value is not null then 1 else 0 end)*dte8.month_length))::numeric(20,6) 
			end as value,
		ind.ss, ind.duch_id, ind.nod_id, ind.dir_id, ind.vids_id, ind.kato_id, ind.depo_id, ind.dep_id
from ( 	select * from dm_stg.calc_src_indicators_t 
...
and ind.dep_id = ind8.dep_id
	...
	join dm_stg.d_date_t dte6 on ind6.dt = dte6.dt
	join dm_stg.d_date_t dte7 on ind7.dt = dte7.dt
	join dm_stg.d_date_t dte8 on ind8.dt = dte8.dt
	where dte.%s
	) a
left join (-- Секция выявления более приоритетных дублей
	select  *
...
# Сверка кол-ва записей
query_2_2_6_17 = ("""select 'tgt', count(0)
		from dm_rep.dm_all_indicators_v clc
		join (select distinct id, group_type from dm_stg.d_hcode_t) hcd
			on clc.hcode_id = hcd.id
where 	calc_rule = 'growplan'
...
query_2_2_6_18 = ("""select 	a.hcode_id, a.hcode_name, a.hcode_unit_name, a.org_id, a.dor_kod, a.date_type_id, a.metric_type_id, a.cargo_type_id, 
		a.val_type_id, a.unit_id, a.dt, a.value, a.ss, a.duch_id, a.nod_id, a.dir_id, a.vids_id, a.kato_id, a.depo_id, a.dep_id
from (
select 	ind.hcode_id, ind.hcode_name, ind.hcode_unit_name, ind.org_id, ind.dor_kod, 3 as date_type_id, ind.metric_type_id, ind.cargo_type_id, 
		1 as val_type_id, ind.unit_id, dte.dt, 
		case
			...
			then 
				...
				+ case when ind9.value is not null then 1 else 0 end))::numeric(20,6) 
			else
				((ind.value*dte.month_length + ind2.value*dte2.month_length + ind3.value*dte3.month_length
				 + ind4.value*dte4.month_length + ind5.value*dte5.month_length + ind6.value*dte6.month_length
				...
				+ (case when ind9.value is not null then 1 else 0 end)*dte9.month_length))::numeric(20,6) 
			end as value,
		ind.ss, ind.duch_id, ind.nod_id, ind.dir_id, ind.vids_id, ind.kato_id, ind.depo_id, ind.dep_id
from ( 	select * from dm_stg.calc_src_indicators_t 
...
and ind.dep_id = ind9.dep_id
	...
	join dm_stg.d_date_t dte7 on ind7.dt = dte7.dt
	join dm_stg.d_date_t dte8 on ind8.dt = dte8.dt
	join dm_stg.d_date_t dte9 on ind9.dt = dte9.dt
	where dte.%s
	) a
left join (-- Секция выявления более приоритетных дублей
	select  *
...
# Сверка кол-ва записей
query_2_2_6_19 = ("""select 'tgt', count(0)
		from dm_rep.dm_all_indicators_v clc
		join (select distinct id, group_type from dm_stg.d_hcode_t) hcd
			on clc.hcode_id = hcd.id
where 	calc_rule = 'growplan'
...
query_2_2_6_20 = ("""select 	a.hcode_id, a.hcode_name, a.hcode_unit_name, a.org_id, a.dor_kod, a.date_type_id, a.metric_type_id, a.cargo_type_id, 
		a.val_type_id, a.unit_id, a.dt, a.value, a.ss, a.duch_id, a.nod_id, a.dir_id, a.vids_id, a.kato_id, a.depo_id, a.dep_id
from (
select 	ind.hcode_id, ind.hcode_name, ind.hcode_unit_name, ind.org_id, ind.dor_kod, 3 as date_type_id, ind.metric_type_id, ind.cargo_type_id, 
		1 as val_type_id, ind.unit_id, dte.dt, 
		case
			...
			then 
				((ind.value + ind2.value + ind3.value + ind4.value + ind5.value 
				+ ind6.value + ind7.value + ind8.value + ind9.value + ind10.value)/
				(case when ind.value is not null then 1 else 0 end + case when ind2.value is not null then 1 else 0 end 
				+ case when ind3.value is not null then 1 else 0 end + case when ind4.value is not null then 1 else 0 end
				+ case when ind5.value is not null then 1 else 0 end + case when ind6.value is not null then 1 else 0 end
				+ case when ind7.value is not null then 1 else 0 end + case when ind8.value is not null then 1 else 0 end
				+ case when ind9.value is not null then 1 else 0 end + case when ind10.value is not null then 1 else 0 end))::numeric(20,6) 
			else
				((ind.value*dte.month_length + ind2.value*dte2.month_length + ind3.value*dte3.month_length
				 + ind4.value*dte4.month_length + ind5.value*dte5.month_length + ind6.value*dte6.month_length
				 + ind7.value*dte7.month_length + ind8.value*dte8.month_length + ind9.value*dte9.month_length
				...
				+ (case when ind10.value is not null then 1 else 0 end)*dte10.month_length))::numeric(20,6) 
			end as value,
		ind.ss, ind.duch_id, ind.nod_id, ind.dir_id, ind.vids_id, ind.kato_id, ind.depo_id, ind.dep_id
from ( 	select * from dm_stg.calc_src_indicators_t 
...
and ind.dep_id = ind10.dep_id
	...
	join dm_stg.d_date_t dte8 on ind8.dt = dte8.dt
	join dm_stg.d_date_t dte9 on ind9.dt = dte9.dt
	join dm_stg.d_date_t dte10 on ind10.dt = dte10.dt
	where dte.%s
	) a
left join (-- Секция выявления более приоритетных дублей
	select  *
...
# Сверка кол-ва записей
query_2_2_6_21 = ("""select 'tgt', count(0)
		from dm_rep.dm_all_indicators_v clc
		join (select distinct id, group_type from dm_stg.d_hcode_t) hcd
			on clc.hcode_id = hcd.id
where 	calc_rule = 'growplan'
...
query_2_2_6_22 = ("""select 	a.hcode_id, a.hcode_name, a.hcode_unit_name, a.org_id, a.dor_kod, a.date_type_id, a.metric_type_id, a.cargo_type_id, 
		a.val_type_id, a.unit_id, a.dt, a.value, a.ss, a.duch_id, a.nod_id, a.dir_id, a.vids_id, a.kato_id, a.depo_id, a.dep_id
from (
select 	ind.hcode_id, ind.hcode_name, ind.hcode_unit_name, ind.org_id, ind.dor_kod, 3 as date_type_id, ind.metric_type_id, ind.cargo_type_id, 
		1 as val_type_id, ind.unit_id, dte.dt, 
		case
			...
			then 
				...
				+ case when ind11.value is not null then 1 else 0 end))::numeric(20,6) 
			else
				((ind.value*dte.month_length + ind2.value*dte2.month_length + ind3.value*dte3.month_length
				 + ind4.value*dte4.month_length + ind5.value*dte5.month_length + ind6.value*dte6.month_length
				 + ind7.value*dte7.month_length + ind8.value*dte8.month_length + ind9.value*dte9.month_length
				...
				+ (case when ind11.value is not null then 1 else 0 end)*dte11.month_length))::numeric(20,6) 
			end as value,
		ind.ss, ind.duch_id, ind.nod_id, ind.dir_id, ind.vids_id, ind.kato_id, ind.depo_id, ind.dep_id
from ( 	select * from dm_stg.calc_src_indicators_t 
...
and ind.dep_id = ind11.dep_id
	...
	join dm_stg.d_date_t dte9 on ind9.dt = dte9.dt
	join dm_stg.d_date_t dte10 on ind10.dt = dte10.dt
	join dm_stg.d_date_t dte11 on ind11.dt = dte11.dt
	where dte.%s
	) a
left join (-- Секция выявления более приоритетных дублей
	select  *
...
# Сверка кол-ва записей
query_2_2_6_23 = ("""select 'tgt', count(0)
		from dm_rep.dm_all_indicators_v clc
		join (select distinct id, group_type from dm_stg.d_hcode_t) hcd
			on clc.hcode_id = hcd.id
where 	calc_rule = 'growplan'
...
query_2_2_6_24 = ("""select 	a.hcode_id, a.hcode_name, a.hcode_unit_name, a.org_id, a.dor_kod, a.date_type_id, a.metric_type_id, a.cargo_type_id, 
		a.val_type_id, a.unit_id, a.dt, a.value, a.ss, a.duch_id, a.nod_id, a.dir_id, a.vids_id, a.kato_id, a.depo_id, a.dep_id
from (
select 	ind.hcode_id, ind.hcode_name, ind.hcode_unit_name, ind.org_id, ind.dor_kod, 3 as date_type_id, ind.metric_type_id, ind.cargo_type_id, 
		1 as val_type_id, ind.unit_id, dte.dt,
		case
			...
			then 
				((ind.value + ind2.value + ind3.value + ind4.value + ind5.value 
				+ ind6.value + ind7.value + ind8.value + ind9.value + ind10.value 
				+ ind11.value + ind12.value)/
				(case when ind.value is not null then 1 else 0 end + case when ind2.value is not null then 1 else 0 end 
				+ case when ind3.value is not null then 1 else 0 end + case when ind4.value is not null then 1 else 0 end
				+ case when ind5.value is not null then 1 else 0 end + case when ind6.value is not null then 1 else 0 end
				+ case when ind7.value is not null then 1 else 0 end + case when ind8.value is not null then 1 else 0 end
				+ case when ind9.value is not null then 1 else 0 end + case when ind10.value is not null then 1 else 0 end
				+ case when ind11.value is not null then 1 else 0 end + case when ind12.value is not null then 1 else 0 end))::numeric(20,6) 
			else
				((ind.value*dte.month_length + ind2.value*dte2.month_length + ind3.value*dte3.month_length
				 + ind4.value*dte4.month_length + ind5.value*dte5.month_length + ind6.value*dte6.month_length
				 + ind7.value*dte7.month_length + ind8.value*dte8.month_length + ind9.value*dte9.month_length
				...
				+ (case when ind12.value is not null then 1 else 0 end)*dte12.month_length))::numeric(20,6) end as value, 
		ind.ss, ind.duch_id, ind.nod_id, ind.dir_id, ind.vids_id, ind.kato_id, ind.depo_id, ind.dep_id
from ( 	select * from dm_stg.calc_src_indicators_t 
...
and ind.dep_id = ind12.dep_id
	...
	join dm_stg.d_date_t dte10 on ind10.dt = dte10.dt
	join dm_stg.d_date_t dte11 on ind11.dt = dte11.dt
	join dm_stg.d_date_t dte12 on ind12.dt = dte12.dt
	where dte.%s
	) a
left join (-- Секция выявления более приоритетных дублей
	select  *
...
QUERYS_2_6 = [v for v in locals() if v.startswith('query')]

QUERYS_2_6_EQUAL = [n for n in QUERYS_2_6 if QUERYS_2_6.index(n) % 2 == 0]
QUERYS_2_6_EMPTY = [n for n in QUERYS_2_6 if QUERYS_2_6.index(n) % 2 != 0]
