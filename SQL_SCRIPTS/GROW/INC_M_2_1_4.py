# ------------------------------------------------------------------------------
# ---- 2.1.4 Нарастающий итог по месяцам с начала квартала. Среднесуточный. ----
# ------------------------------------------------------------------------------
...
query_2_1_4_1 = ("""select 'calc', count(0) from dm_rep.dm_all_indicators_v calc
	...
	where 	hcd.group_type = 'среднесуточный'
		...
		and calc.val_type_id = 9 -- Нарастающий итог по месяцам с начала квартала
		and calc.date_type_id = 4 -- Месяц
...
select 'src', count(0) from (
	...
	where 	hcd.group_type = 'среднесуточный'
		...
		and ind.val_type_id = 7 -- Итог
		and ind.date_type_id = 4 -- Месяц
		and dte.month_from_quater = 1
		and ind.%s
		) a
left join ( -- Секция выявления более приоритетных дублей
	select  *
		from (select * from dm_rep.dm_all_indicators_v where vt = 'BASE') ind 
...
query_2_1_4_2 = ("""select 	calc.hcode_id, org_id, dor_kod, date_type_id, metric_type_id, cargo_type_id, val_type_id, calc.unit_id, calc.dt, 
		ss, value, duch_id, nod_id, dir_id, vids_id, kato_id, depo_id, dep_id
	...
	join dm_stg.d_date_t dte
		on dte.dt = calc.dt	
	where 	hcd.group_type = 'среднесуточный'
		and dte.month_from_quater = 1 -- Второй месяц квартала
		and calc.val_type_id = 9 -- Нарастающий итог по месяцам с начала квартала
		and calc.date_type_id = 4 -- Месяц
		and calc.calc_rule = 'grow'
		and calc.%s
...
from (
	select 	ind.hcode_id, ind.org_id, ind.dor_kod, 4 as date_type_id, ind.metric_type_id, ind.cargo_type_id, 9 as val_type_id, ind.unit_id, 
			ind.dt, ind.ss, value, ind.duch_id, ind.nod_id, ind.dir_id, ind.vids_id, ind.kato_id, ind.depo_id, ind.dep_id
			from dm_rep.dm_all_indicators_v ind
			join (select distinct id, group_type from dm_stg.d_hcode_t) hcd
				on hcd.id = ind.hcode_id
			join dm_stg.d_date_t dte
...
# Сверка кол-ва записей
query_2_1_4_3 = ("""select 'calc', count(0) 
	from dm_rep.dm_all_indicators_v calc
	join (select distinct id, group_type from dm_stg.d_hcode_t) hcd
		on hcd.id = calc.hcode_id
	join dm_stg.d_date_t dte
...
# Сравнение всех записей в таблицах
query_2_1_4_4 = ("""select 	calc.hcode_id, org_id, dor_kod, date_type_id, metric_type_id, cargo_type_id, val_type_id, calc.unit_id, calc.dt, ss, value, 
...
from (
	select 	ind.hcode_id, ind.org_id, ind.dor_kod, 4 as date_type_id, ind.metric_type_id, ind.cargo_type_id, 9 as val_type_id, ind.unit_id, 
			ind.dt, ind.ss, ((ind.value * dte.month_length + ind2.value * dte2.month_length) / 
			(case when ind.value is null then 0 else dte.month_length end +case when ind2.value is null then 0 else dte2.month_length end)) 
			as value, ind.duch_id, ind.nod_id, ind.dir_id, ind.vids_id, ind.kato_id, ind.depo_id, ind.dep_id
			from dm_rep.dm_all_indicators_v ind
			left join dm_rep.dm_all_indicators_v ind2 -- Для итога предыдущего месяца
...
# Сверка кол-ва записей
query_2_1_4_5 = ("""select 'calc', count(0) from dm_rep.dm_all_indicators_v calc
	join (select distinct id, group_type from dm_stg.d_hcode_t) hcd
		on hcd.id = calc.hcode_id
	join dm_stg.d_date_t dte
		on dte.dt = calc.dt	
...
# Сравнение всех записей в таблицах
query_2_1_4_6 = ("""select 	a.hcode_id, a.org_id, a.dor_kod, a.date_type_id, a.metric_type_id, a.cargo_type_id, 
		a.val_type_id, a.unit_id, a.dt, a.ss, a.value, a.duch_id, a.nod_id, a.dir_id, a.vids_id, a.kato_id, a.depo_id, a.dep_id
from (
	select 	ind.hcode_id, ind.org_id, ind.dor_kod, 4 as date_type_id, ind.metric_type_id, ind.cargo_type_id, 9 as val_type_id, ind.unit_id, 
			ind.dt, ind.ss, ((ind.value * dte.month_length + ind2.value * dte2.month_length 
			+ ind3.value * dte3.month_length) / (case when ind.value is null then 0 else dte.month_length end 
			+ case when ind2.value is null then 0 else dte2.month_length end + case when ind3.value is null then 0 
			else dte3.month_length end)) as value, ind.duch_id, ind.nod_id, ind.dir_id, ind.vids_id, ind.kato_id, ind.depo_id, ind.dep_id
			from dm_rep.dm_all_indicators_v ind
			left join dm_rep.dm_all_indicators_v ind2 -- Для итога первого месяца квартала месяца
				on (ind.hcode_id = ind2.hcode_id
					and ind.org_id = ind2.org_id
					and ind.cargo_type_id = ind2.cargo_type_id
					and ind.unit_id = ind2.unit_id
...
and ind.dep_id = ind2.dep_id
					...
					and date_part('month', ind.dt) = (date_part('month', ind2.dt)+1)
					and date_part('year', ind.dt) = date_part('year', ind2.dt))
			left join dm_rep.dm_all_indicators_v ind3 -- Для итога второго месяца квартала
...
QUERYS_2_1_4 = [v for v in locals() if v.startswith('query')]

QUERYS_2_1_4_EQUAL = [n for n in QUERYS_2_1_4 if QUERYS_2_1_4.index(n) % 2 == 0]
QUERYS_2_1_4_EMPTY = [n for n in QUERYS_2_1_4 if QUERYS_2_1_4.index(n) % 2 != 0]