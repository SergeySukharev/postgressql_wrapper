# ----------------------------------------------------
# ---- Квартал. Расчетный поправочный коэффициент ----
# ----------------------------------------------------
...
and a.dep_id = c.dep_id
				and a.dt = c.dt)
where 	a.val_type_id in (4, 7)
	and a.date_type_id = 2 -- Квартал
	and a.%s
...
# ----------------------------------
query_5_3_2 = ("""select 	hcode_id, hcode_name, hcode_unit_name, org_id, dor_kod, date_type_id, metric_type_id, cargo_type_id, val_type_id, unit_id, 
...
# -------------------------------------------------------------------------------------------------------------------------------------------
# ---- Квартал. Влияние отклонения факта грузооборота брутто к плану на производительность локомотива рабочего парка в грузовом движении ----
# -------------------------------------------------------------------------------------------------------------------------------------------
...
union all
select 'join', a.val_type_id, count(0)
		from (select * from dm_rep.dm_all_indicators_v where hcode_id = '00012' and metric_type_id = 17) a
		left join (select * from dm_rep.dm_all_indicators_v where hcode_id = '00034' and metric_type_id = 12) b
			on (	a.org_id = b.org_id
				and a.dor_kod = b.dor_kod
				and a.date_type_id = b.date_type_id
				and a.cargo_type_id = b.cargo_type_id
...
and a.dep_id = b.dep_id
				and a.dt = b.dt)
		left join (select * from dm_rep.dm_all_indicators_v where hcode_id = '00033' and metric_type_id = 17) c
			on (	a.org_id = c.org_id
				and a.dor_kod = c.dor_kod
				and a.date_type_id = c.date_type_id
				and a.cargo_type_id = c.cargo_type_id
...
# -- Сравнение записей в таблицах --
# ----------------------------------
query_5_3_4 = ("""select hcode_id, hcode_name, hcode_unit_name, org_id, dor_kod, date_type_id, metric_type_id, cargo_type_id, 
...
# --------------------------------------------------------------------------------------------------------------------------------------
# ---- Квартал. Влияние отклонения факта рабочего парка к плану на производительность локомотива рабочего парка в грузовом движении ----
# --------------------------------------------------------------------------------------------------------------------------------------
...
query_5_3_5 = ("""select * from (
	...
	select 'join', a.val_type_id, count(0)
		from (select * from dm_rep.dm_all_indicators_v where hcode_id = '00012' and metric_type_id = 17) a
		left join (select * from dm_rep.dm_all_indicators_v where hcode_id = '00034' and metric_type_id = 17) b
			on (	a.org_id = b.org_id
				and a.dor_kod = b.dor_kod
				and a.date_type_id = b.date_type_id
				and a.cargo_type_id = b.cargo_type_id
...
and a.dep_id = b.dep_id
				and a.dt = b.dt)
		left join (select * from dm_rep.dm_all_indicators_v where hcode_id = '00033' and metric_type_id = 12) c
			on (	a.org_id = c.org_id
				and a.dor_kod = c.dor_kod
				and a.date_type_id = c.date_type_id
				and a.cargo_type_id = c.cargo_type_id
...
query_5_3_6 = ("""select 	hcode_id, hcode_name, hcode_unit_name, org_id, dor_kod, date_type_id, metric_type_id, cargo_type_id, 
		val_type_id, unit_id, dt, value, ss, duch_id, nod_id, dir_id, kato_id, vids_id, depo_id, dep_id  from dm_rep.dm_all_indicators_v
where 	calc_rule = 'influence'
	and hcode_id in ('00038') -- Влияние отклонения факта грузооборота брутто к плану на производительность локомотива рабочего парка в грузовом движении
	and val_type_id in (4, 7)
...
# -------------------------------------------------------------------------------------------------
# ---- Квартал. Влияние отклонения факта среднего состава поезда к плану на средний вес поезда ----
# -------------------------------------------------------------------------------------------------
...
query_5_3_7 = ("""select * from (
	...
	union all
	select 'join', a.val_type_id, count(0)
		from (select * from dm_rep.dm_all_indicators_v where hcode_id = '00011' and metric_type_id = 17) a
		left join (select * from dm_rep.dm_all_indicators_v where hcode_id = '00031' and metric_type_id = 17) b
			on (	a.org_id = b.org_id
				and a.dor_kod = b.dor_kod
				and a.date_type_id = b.date_type_id
				and a.cargo_type_id = b.cargo_type_id
...
and a.dep_id = b.dep_id
				and a.dt = b.dt)
		left join (select * from dm_rep.dm_all_indicators_v where hcode_id = '00028' and metric_type_id = 12) c
			on (	a.org_id = c.org_id
				and a.dor_kod = c.dor_kod
				and a.date_type_id = c.date_type_id
				and a.cargo_type_id = c.cargo_type_id
...
query_5_3_8 = ("""select 	hcode_id, hcode_name, hcode_unit_name, org_id, dor_kod, date_type_id, metric_type_id, cargo_type_id, 
		val_type_id, unit_id, dt, value, ss, duch_id, nod_id, dir_id, kato_id, vids_id, depo_id, dep_id  from dm_rep.dm_all_indicators_v
where 	calc_rule = 'influence'
	and hcode_id in ('00039') -- Влияние отклонения факта среднего состава поезда к плану на средний вес поезда
	and val_type_id in (4, 7)
...
# -------------------------------------------------------------------------------------------------------------
# ---- Квартал. Влияние отклонения факта процента порожнего пробега к общему к плану на средний вес поезда ----
# -------------------------------------------------------------------------------------------------------------
...
query_5_3_9 = ("""select * from (
	...
	select 'join', a.val_type_id, count(0)
		from (select * from dm_rep.dm_all_indicators_v where hcode_id = '00011' and metric_type_id = 17) a
		left join (select * from dm_rep.dm_all_indicators_v where hcode_id = '00032' and metric_type_id = 12) b
			on (	a.org_id = b.org_id
				and a.dor_kod = b.dor_kod
				and a.date_type_id = b.date_type_id
				and a.cargo_type_id = b.cargo_type_id
...
and a.dep_id = b.dep_id
				and a.dt = b.dt)
		left join (select * from dm_rep.dm_all_indicators_v where hcode_id = '00036' and metric_type_id = 17) c
			on (	a.org_id = c.org_id
				and a.dor_kod = c.dor_kod
				and a.date_type_id = c.date_type_id
				and a.cargo_type_id = c.cargo_type_id
...
and a.dep_id = c.dep_id
				and a.dt = c.dt)
		left join (select * from dm_rep.dm_all_indicators_v where hcode_id = '00030' and metric_type_id = 17) d
...
and a.dep_id = d.dep_id
				and a.dt = d.dt)
		left join (select * from dm_rep.dm_all_indicators_v where hcode_id = '00028' and metric_type_id = 17) e
...
query_5_3_10 = ("""select 	hcode_id, hcode_name, hcode_unit_name, org_id, dor_kod, date_type_id, metric_type_id, cargo_type_id, 
		val_type_id, unit_id, dt, value, ss, duch_id, nod_id, dir_id, kato_id, vids_id, depo_id, dep_id  from dm_rep.dm_all_indicators_v
where 	calc_rule = 'influence'
	and hcode_id in ('00040') -- Влияние отклонения факта процента порожнего пробега к общему к плану на средний вес поезда
	and val_type_id in (4, 7)
...
# --------------------------------------------------------------------------------------------------------------------------------------------
# ---- Квартал. Влияние отклонения факта средней динамической нагрузки на рабочий вагон в грузовом движении к плану на средний вес поезда ----
# --------------------------------------------------------------------------------------------------------------------------------------------
...
query_5_3_11 = ("""select * from (
	...
	select 'join', a.val_type_id, count(0)
		from (select * from dm_rep.dm_all_indicators_v where hcode_id = '00011' and metric_type_id = 17) a
		left join (select * from dm_rep.dm_all_indicators_v where hcode_id = '00030' and metric_type_id = 17) b
			on (	a.org_id = b.org_id
				and a.dor_kod = b.dor_kod
				and a.date_type_id = b.date_type_id
				and a.cargo_type_id = b.cargo_type_id
...
and a.dep_id = b.dep_id
				and a.dt = b.dt)
		left join (select * from dm_rep.dm_all_indicators_v where hcode_id = '00029' and metric_type_id = 12) c
			on (	a.org_id = c.org_id
				and a.dor_kod = c.dor_kod
				and a.date_type_id = c.date_type_id
				and a.cargo_type_id = c.cargo_type_id
...
and a.dep_id = c.dep_id
				and a.dt = c.dt)
		left join (select * from dm_rep.dm_all_indicators_v where hcode_id = '00028' and metric_type_id = 17) d
		on (	a.org_id = d.org_id
				and a.dor_kod = d.dor_kod
				and a.date_type_id = d.date_type_id
				and a.cargo_type_id = d.cargo_type_id
...
and a.dep_id = d.dep_id
				and a.dt = d.dt)
	where 	a.date_type_id = 2 -- Квартал
		and a.val_type_id in (4, 7)
		and a.%s
group by a.val_type_id
)al 
order by al.val_type_id""")
...
query_5_3_12 = ("""select 	hcode_id, hcode_name, hcode_unit_name, org_id, dor_kod, date_type_id, metric_type_id, cargo_type_id, 
		val_type_id, unit_id, dt, value, ss, duch_id, nod_id, dir_id, kato_id, vids_id, depo_id, dep_id from dm_rep.dm_all_indicators_v
where 	calc_rule = 'influence'
	and hcode_id in ('00041') -- Влияние отклонения факта средней динамической нагрузки на рабочий вагон в грузовом движении к плану на средний вес поезда
	and val_type_id in (4, 7)
...
# -------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# ---- Квартал. Влияние отклонения факта среднесуточного пробега локомотива рабочего парка к плану на производительность локомотива рабочего парка в грузовом движении ----
# -------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
query_5_3_13 = ("""select * from (
	...
	select 'join', a.val_type_id, count(0)
		from (select * from dm_rep.dm_all_indicators_v where hcode_id = '00012' and metric_type_id = 17) a
		left join (select * from dm_rep.dm_all_indicators_v where hcode_id = '00011' and metric_type_id = 17) b
			on (	a.org_id = b.org_id
				and a.dor_kod = b.dor_kod
				and a.date_type_id = b.date_type_id
				and a.cargo_type_id = b.cargo_type_id
...
and a.dep_id = b.dep_id
				and a.dt = b.dt)
		left join (select * from dm_rep.dm_all_indicators_v where hcode_id = '00035' and metric_type_id = 29) c
			on (	a.org_id = c.org_id
				and a.dor_kod = c.dor_kod
				and a.date_type_id = c.date_type_id
				and a.cargo_type_id = c.cargo_type_id
...
and a.dep_id = c.dep_id
				and a.dt = c.dt)
		left join (select * from dm_rep.dm_all_indicators_v where hcode_id = '00027' and metric_type_id = 12) d
		on (	a.org_id = d.org_id
				and a.dor_kod = d.dor_kod
				and a.date_type_id = d.date_type_id
				and a.cargo_type_id = d.cargo_type_id
...
query_5_3_14 = ("""select 	hcode_id, hcode_name, hcode_unit_name, org_id, dor_kod, date_type_id, metric_type_id, cargo_type_id, 
		val_type_id, unit_id, dt, value, ss, duch_id, nod_id, dir_id, kato_id, vids_id, depo_id, dep_id  from dm_rep.dm_all_indicators_v
where 	calc_rule = 'influence'
	and hcode_id in ('00042') 
	and val_type_id in (4, 7)
...
QUERYS_5_3 = [v for v in locals() if v.startswith('query')]

QUERYS_5_3_EQUAL = [n for n in QUERYS_5_3 if QUERYS_5_3.index(n) % 2 != 0]
QUERYS_5_3_EMPTY = [n for n in QUERYS_5_3 if QUERYS_5_3.index(n) % 2 == 0]
