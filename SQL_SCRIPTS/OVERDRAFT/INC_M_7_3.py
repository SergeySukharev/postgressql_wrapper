...
# --CD where metric_type =17 and hcode=00100 - (CD where metric_type=17 and hcode=00291 *1000 / CD where metric_type=12 and hcode=00224/количество дней в квартале). Если одной из составляющих нет, то NULL
	
# -- Сравнение количества записей в таблицах --
# ---------------------------------------------

query_7_3_1 = ("""select * from (
...
query_7_3_2 = ("""select 	hcode_id, hcode_name, hcode_unit_name, org_id, dor_kod, date_type_id, metric_type_id, cargo_type_id, 
...
select 	'00293' as hcode_id, 
		...
		a.val_type_id as val_type_id, a.unit_id as unit_id, a.dt as dt, (a.value - nullif(b.value,0) * 1000/nullif(c.value,0)/(coalesce(a.value,0)*dte.day_of_month + coalesce(a.value,0)*dte2.month_length + coalesce(a.value,0) 
		* dte3.month_length)) as value, a.ss as ss, a.duch_id, a.nod_id, a.dir_id, a.kato_id, a.vids_id, a.depo_id, a.dep_id
		from (select * from dm_rep.dm_all_indicators_v where hcode_id = '00100' and metric_type_id = 17) a
		left join (select * from dm_rep.dm_all_indicators_v where hcode_id = '00291' and metric_type_id = 17) b
			on (	a.org_id = b.org_id
				and a.dor_kod = b.dor_kod
...
and a.dep_id = c.dep_id
		...
		join dm_stg.d_date_t dte3
			on dte3.dt = (date(a.dt) - interval '2 month')
where 	a.val_type_id in (4,7)
	and a.date_type_id = 2 -- Квартал
	and a.%s""")
...
# --CD where metric_type =17 and hcode=00097 - (CD where metric_type=17 and hcode=00292 *1000 / CD where metric_type=12 and hcode=00224/количество дней в квартале). Если одной из составляющих нет, то NULL

# -- Сравнение количества записей в таблицах --
# ---------------------------------------------

query_7_3_3 = ("""select * from (
select 'calc', val_type_id, count(0) from dm_rep.dm_all_indicators_v
...
query_7_3_4 = ("""select 	hcode_id, hcode_name, hcode_unit_name, org_id, dor_kod, date_type_id, metric_type_id, cargo_type_id, 
		val_type_id, unit_id, dt, value, ss, duch_id, nod_id, dir_id, kato_id, vids_id, depo_id, dep_id
	from dm_rep.dm_all_indicators_v
where hcode_id in ('00294')
	and val_type_id in (4,7)
...
# --CD where metric_type =17 and hcode=00081 - (CD where metric_type=17 and hcode=00291 *1000 / CD where metric_type=12 and hcode=00012/количество дней в квартале). Если одной из составляющих нет, то NULL

# -- Сравнение количества записей в таблицах --
# ---------------------------------------------

query_7_3_5 = ("""select * from (
select 'calc', val_type_id, count(0) from dm_rep.dm_all_indicators_v
...
query_7_3_6 = ("""select 	hcode_id, hcode_name, hcode_unit_name, org_id, dor_kod, date_type_id, metric_type_id, cargo_type_id, 
		val_type_id, unit_id, dt, value, ss, duch_id, nod_id, dir_id, kato_id, vids_id, depo_id, dep_id
	from dm_rep.dm_all_indicators_v
where hcode_id in ('00295')
	and val_type_id in (4,7)
...
# --CD where metric_type =17 and hcode=00105 - (CD where metric_type=17 and hcode=00292 *1000 / CD where metric_type=12 and hcode=00012/количество дней в квартале). Если одной из составляющих нет, то NULL

# -- Сравнение количества записей в таблицах --
# ---------------------------------------------

query_7_3_7 = ("""select * from (
select 'calc', val_type_id, count(0) from dm_rep.dm_all_indicators_v
...
query_7_3_8 = ("""select 	hcode_id, hcode_name, hcode_unit_name, org_id, dor_kod, date_type_id, metric_type_id, cargo_type_id, 
		val_type_id, unit_id, dt, value, ss, duch_id, nod_id, dir_id, kato_id, vids_id, depo_id, dep_id
	from dm_rep.dm_all_indicators_v
where hcode_id in ('00296')
	and val_type_id in (4,7)
...
QUERYS_7_3 = [v for v in locals() if v.startswith('query')]

QUERYS_7_3_EQUAL = [n for n in QUERYS_7_3 if QUERYS_7_3.index(n) % 2 != 0]
QUERYS_7_3_EMPTY = [n for n in QUERYS_7_3 if QUERYS_7_3.index(n) % 2 == 0]
